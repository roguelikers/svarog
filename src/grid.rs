...
impl Grid {
    pub fn from(mapping: HashMap<String, String>) -> Option<Self> {
        ...
        let Some(kind) = mapping.get(&"kind".to_string()) else { println!("KIND!"); return None; };
        let Some(kind) = GridKind::from(kind.as_str()) else { println!("KIND?"); return None; };
        
        let Some(grid_w) = mapping.get(&"grid size x".to_string()) else { println!("GW"); return None; };
        let Ok(grid_w) = grid_w.parse::<i32>() else { println!("GW?"); return None; };
        let Some(grid_h) = mapping.get(&"grid size y".to_string()) else { println!("GH"); return None; };
        let Ok(grid_h) = grid_h.parse::<i32>() else { println!("GH?"); return None; };
...
fn initialize_grids(mut commands: Commands, mut grids: ResMut<Grids>, grid_config: Res<GridsConfig>) {
    ...
    let loaded_grids = reader.records().filter_map(|record| {
